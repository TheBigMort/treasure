...
describe('Test all', () => {
    ...
    describe('Mint Reverts', () => {
        ...
        it('Error: Not enough ether', async () => {
            ...
            try {
                await ac.plunder(num, { value: parseEther((parseInt(conParams.price)-0.000001).toString()).mul(num) });
                // eslint-disable-next-line no-empty
            } catch {
            } finally {
                expect((await ac.balanceOf(cs.address)).toNumber()).to.equal(0);
    ...
    describe('Functions', () => {
        ...
        it('mintReserve', async () => {
            expect((await t4w.balanceOf(signers[0].address)).toNumber()).to.equal(25);
            expect((await t4w.reserved()).toNumber()).to.equal(0);
            await t4w.mintReserve(5);
            expect((await t4w.balanceOf(signers[0].address)).toNumber()).to.equal(30);
            expect((await t4w.reserved()).toNumber()).to.equal(5);
        });
        it('setBaseURI', async () => {
            const newURI: string = 'testing/';
            // eslint-disable-next-line no-underscore-dangle
            expect(await t4w.tokenURI(1)).to.equal(`${conArgs.__uri}1`);
            expect(await t4w.baseURI()).to.equal(conArgs.__uri);
        ...
        it('withdraw', async () => {
            ...
            const acctInitBal: number[] = (
                await Promise.all(
                    (<Array<string>>conArgs.payees).map((address) => {
        ...
        it('ownerOf', async () => {
            expect(await t4w.ownerOf(getRandInt(25))).to.equal(signers[0].address);
        });
        it('name', async () => {
        ...
        it('tokenOfOwnerByIndex', async () => {
            const randIndex: number = getRandInt(25);

            expect((await t4w.tokenOfOwnerByIndex(signers[0].address, randIndex)).toNumber()).to.equal(
                randIndex
            );
        });
        it('tokenByIndex', async () => {
            const randIndex: number = getRandInt(25);
            expect((await t4w.tokenByIndex(randIndex)).toNumber()).to.equal(randIndex);
        });
        it('tokensOfOwner', async () => {
            (await t4w.tokensOfOwner(signers[0].address)).forEach(
                (elem: BigNumber, index: number) => expect(elem.toNumber()).to.equal(index)
            );
        });
