...
const buildMint = (con: Contract, signer: SignerWithAddress, numMints: number) => {
    const overrides = {
    ...
    if (Array.isArray(tx)) {
        return (
            await Promise.all(
                ...
                ).map((t: TransactionResponse) => hre.ethers.provider.getTransactionReceipt(t.hash))
            )
        ).map((txRec: TransactionReceipt) => Math.round(txRec.gasUsed.toNumber() * 100) / 100);
    }
    const t: TransactionResponse = await tx;
    return (
        Math.round(
            (await hre.ethers.provider.getTransactionReceipt(t.hash)).gasUsed.toNumber() * 100
        ) / 100
    );
};
...
describe('Gas Tests', () => {
    ...
    const getRandSigner: Function = (): SignerWithAddress => signers[getRandInt(signers.length)];
    beforeEach(async () => {
        try {
    ...
    describe('Mints', () => {
        ...
        for (let i = 1; i <= 10; i += 1) {
            ...
            it(title, async () => {
                ...
                for (let k = 0; k < Math.floor(conParams.MAX_RESERVE / numMints); k += 1) {
                    txs.push(
                        lbr.connect(signers[0]).mintReserveToAddress(numMints, signers[5].address)
                    );
                }
    ...
    describe('withdrawals', async () => {
        ...
        it('release', async () => {
            const txs: Promise<TransactionResponse>[] = [signers[4], signers[5], signers[6]].map(
                (s: SignerWithAddress) => lbr.connect(s).release(s.address)
            );
...
after(() => {
    ...
    setCSVStr();
    Array.from(gasMap.entries()).forEach((val: [string, number]) => setCSVStr(val[0], val[1]));
    const fileNum: number = fs.readdirSync(reportDir).filter((val) => val.endsWith('.csv')).length;
    const csvName: string =
        fileNum === 0 ? `${reportDir}gasUsed.csv` : `${reportDir}gasUsed(${fileNum}).csv`;
