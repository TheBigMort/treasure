...
const Mint: FC = () => {
    ...
    try {
      const transaction = await contract.plunder(mintAmount, {
        value: ethers.utils.parseEther(
          `${parseFloat(conParams.price) * mintAmount}`
        ),
      });
  ...
  const increaseMintAmount = useCallback(() => {
    ...
    setMintAmount((amount) =>
      amount < conParams.MAX_MULTIMINT ? amount + 1 : amount
    );
  }, []);
  return (
    <Stack id="mint" alignItems="center">
      ...
      <Stack>
        ...
        <LoadingButton
          loading={isLoading}
          disabled={mintAmount === 0 || chainId !== chainIndex || minted >= conParams.MAX_SUPPLY}
